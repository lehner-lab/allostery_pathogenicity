---
title: "ESM1v vs. Experimental Fitness Across Protein Domains (Domainome & Mega-scale)"
output: pdf_document
date: "2025-06-14"
---
...
## Load and Preprocess Domainome Data

Read the Domainome 1.0 supplement table, remove variants missing ESM1v scores or fitness measurements, and parse mutation identifiers and positions from the UniProt mutation strings.
...
## Load and Clean Mega-scale Stability Data

Read the Mega-scale (Tsuboyama 2023) ddG dataset, discard insertions/deletions and rows with missing ddG values, then classify each variant into stabilizing, destabilizing, or wild-type-like stability classes.
...
## Filter for Proteins with Available AlphaFold Structures

Assign stability classes and restrict the dataset to wild-type proteins that have a corresponding AlphaFold PDB model on disk.
...
## Merge Mega-scale Data with ProteinGym ESM1v Scores

Join the Mega-scale stability measurements with pre-computed ESM1v ensemble scores from ProteinGym by matching mutated sequences, then write the merged table to disk.
...
## Map PDB Identifiers to UniProt Accessions

Extract unique wild-type protein names, strip the `.pdb` extension, and use a pre-downloaded UniProt ID-mapping table to identify proteins that appear in both the Mega-scale and Domainome datasets.
...
## Identify Overlapping Proteins Between Datasets

Pull the PDB base names corresponding to UniProt accessions shared between Mega-scale and Domainome, then extract one representative sequence per overlapping protein from each dataset.
...
## Extract Domainome Sequences for Overlapping Proteins

Retrieve one representative domain sequence per overlapping UniProt entry from the Domainome dataset for manual sequence comparison.
...
## Cross-reference Domainome and Mega-scale via PDB Map

Join the Domainome overlap sequences with the PDB-to-UniProt mapping table to pair each domain sequence with its corresponding PDB identifier.

```{r}
merge(domainome_data_overlap, pdb_map, by.x="uniprot_ID", by.y = "Entry")
```

## Manual Sequence Overlap Verification

Retrieve paired wild-type sequences from both datasets for each overlapping protein to enable manual alignment checks in tools like AliView.
...
## Remove Overlapping Proteins and Compute Per-domain R²

Exclude Mega-scale proteins whose sequences overlap with the Domainome to avoid double-counting, then fit per-protein linear models between ESM1v scores and ddG values and summarise the distribution with a median R².
...
# Combined R² Distribution Plot

## Visualise ESM1v Predictive Performance Across Both Datasets

Combine the per-domain R² values from Domainome and Mega-scale into a single data frame and produce a violin-plus-boxplot figure comparing the distributions, annotated with domain counts and median R².
...
## Report R² Ranges for Both Datasets

Print the minimum and maximum adjusted R² values observed across domains in each dataset.
...
## Count Human Domains in Mega-scale

Tally how many of the non-overlapping Mega-scale proteins are annotated as human, and compare this to the number of human domains covered by Domainome.
...
## Classify Domainome Variants by Stability Using Z-scores

Compute per-variant z-scores from scaled fitness and its standard error, apply FDR correction, and assign each variant to one of four stability categories (stabilizing, mildly destabilizing, strongly destabilizing, or wild type-like).
...
## Merge Domainome with ClinVar Annotations

Load the proteome-wide ClinVar variant table and join it to the Domainome dataset by UniProt accession and variant identifier to obtain clinically annotated variants with experimental fitness measurements.
...
## Attempted ClinVar Merge for Mega-scale (Commented Out)

This block documents an attempted inner join between the Mega-scale dataset and ClinVar via UniProt accessions; it is kept for reference because the merge yields no overlapping variants.
...
## Recode Stability Classes for Plotting

Collapse the four-level stability classification into three broader groups (stabilizing, destabilizing, WT-like) to simplify the ClinVar stacked-bar figure.
...
Merge likely benign/pathogenic with their definitive counterparts, group by clinical significance and stability, and compute per-group variant counts and percentages for the bar chart.
...
## Plot ClinVar Stability Distribution (Domainome)

Produce a stacked bar chart showing the proportion of stabilizing, destabilizing, and WT-like variants among benign and pathogenic ClinVar entries across 167 human domains, and save it as a PDF panel.
