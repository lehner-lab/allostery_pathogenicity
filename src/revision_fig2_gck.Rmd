...
#color sel grey
#select @@bfactor<999 
#color byattribute a:bfactor #4 & sel target csab palette -1.2,red:0,white:2.6,blue
#select ::name="TLA"
...
p13 <- ggplot(combined_df_int, aes(x = var_source, y = residuals, fill = var_source)) +
  ...
  labs(
    title = "GCK: 71 Orthosteric Variants",
    x = "",
...
# View summary
#nrow(min_dist_df) #448
#summary(min_dist_df$min_dist_to_ligand)
#head(min_dist_df$min_dist_to_ligand)
#length(min_dist_df$min_dist_to_ligand) #448
...
#      a      b 
# 0.8877 0.0437 
#  residual sum-of-squares: 19.6
# 
# Number of iterations to convergence: 5 
# Achieved convergence tolerance: 7.272e-07
```
...
max(merged_df_residue %>% filter (site_type == "Glucose-binding site") %>% pull(min_dist_to_ligand)) #7.786475
...
p23 <- ggplot(merged_df_residue, aes(x = min_dist_to_ligand, y = abs(loess_residual_avg))) +
  ...
  theme_classic() +
  geom_vline(xintercept = 7.786475, linetype = "dashed", color = "slategrey") +
  labs(
...
geom_text_repel(
  ...
  scale_color_manual(values = c(
    #"Null" = "grey",
    "Non-orthosteric site" = "#4E7D6B",
    "ATP-binding site" = "#6A4C93",
    "Glucose-binding site" = "#E69F00"
  )) +
...
ggsave("/Users/xl7/Documents/0.Projects/01.protein-seq-evo-v1/figs/revision/fig2_decay2.pdf", 
       plot = p24, width = 4, height = 4, dpi = 300)
```
