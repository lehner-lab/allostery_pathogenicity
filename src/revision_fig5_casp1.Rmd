...
doubledeepms__minimum_interchain_distances_from_PDB_perres <- function(
  ...
  #    We accept EITHER the resid_target OR the chain_target
  ...
  # A. Heavy Atoms (HA): Everything NOT H and NOT Water
  sele_HA <- bio3d::combine.select(sele_H, sele_water, operator = "-", verbose=FALSE)

  # B. Side Chain Heavy Atoms (scHA)
  #    Standard Protein Sidechains (or Gly Ca)
  ...
  #    Remove H/Water from protein parts
  sele_scHA_prot     <- bio3d::combine.select(sele_prot_sc_glyca, sele_H, sele_water, operator = "-", verbose=FALSE)
  
  #    CRITICAL STEP: Add the Target Atoms (Ligand + Chain C) to the scHA selection
  #    Otherwise, the matrix will only contain protein atoms, and the target columns will be missing.
  ...
  #    Final scHA = (Protein Sidechains) OR (Target Heavy Atoms)
  ...
  for(metric in names(sele_list)){
    ...
    # Identify Rows (Query) and Cols (Target)
    query_inds  <- resno_sub[,"chain"] %in% chain_query
    
    # Target Logic: Chain matches target OR Residue Name matches target
    # Use trimws to avoid whitespace mismatches (e.g. "PHQ " vs "PHQ")
    target_inds <- (resno_sub[,"chain"] %in% chain_target) | (trimws(resno_sub[,"resid"]) %in% resid_target)

    # SAFETY CHECK: Ensure we actually found the targets
    if(sum(target_inds) == 0) {
      ...
      # We create a dummy result for the query atoms so the merge doesn't break
      min_vals <- rep(Inf, sum(query_inds))
    } else {
      # Subset the matrix: Rows=Query, Cols=Target
      ligand_dist <- dist_mat[query_inds, target_inds, drop=FALSE]
      
      # Calculate Min Dist per row
