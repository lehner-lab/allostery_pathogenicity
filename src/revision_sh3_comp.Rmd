...
wt_sequence <- sh3_final_df_ddgf %>%
  # 1. Remove the control row (id = "WT") and any rows where Position is NA
  filter(id != "WT", !is.na(Pos)) %>%
  
  # 2. Extract the first letter of the 'id' column (this is the Wild Type AA)
  #    and keep the Position column
  mutate(wt_aa = substr(id, 1, 1)) %>%
  dplyr::select(Pos, wt_aa) %>%
  
  # 3. Remove duplicates (since you have multiple mutations per position, e.g., T1A, T1C...)
  #    We only need one reference per position.
  distinct() %>%
  
  # 4. Ensure positions are in the correct numerical order (1, 2, 3...)
  arrange(Pos) %>%
  
  # 5. Extract the amino acids and collapse them into a single string
  pull(wt_aa) %>%
  paste0(collapse = "")
